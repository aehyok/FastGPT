...
const Dialogue = ({
  ...
  const gptChatPrompt = useCallback(
    async (prompts: ChatSiteItemType[]) => {
      ...
      // }
      if (
        ['parse-requirements-and-objectives-eng', 'parse-requirements-and-objectives-cmn'].includes(
          type
        )
      ) {
        const languag = type === 'parse-requirements-and-objectives-cmn' ? '中文' : '英文';
        prompt = {
          obj: prompts[0].obj,
          // 原来
          ...
          // `
          value: `现在你是一名人工客服，你为一家主营手机业务的亚马逊线上商店服务，我从现在开始会给你发信息，你要告诉我这段信息客户的实际需求或者实际目的是什么，并告诉我它的手机有什么问题，请将这些用最简短的语言总结出来，你的第一段信息是: 现在你是一名人工客服，你为一家主营手机业务的亚马逊线上商店服务，我从现在开始会给你发信息，你要告诉我这段信息客户的实际需求，请将他需求用最简短的语言总结出来，总结内容不需要包括手机型号，具体日期、情感等，并且以最简洁的方式回复我，并且按照客户需求+手机问题的模版回复我，
       
      
          现在请解析以下内容并用${languag}将客户需求和手机问题两个方面进行输出（无需输出多余内容）：${prompts[0].value} `
        };
      } else {
        prompt = {
          obj: prompts[0].obj,
