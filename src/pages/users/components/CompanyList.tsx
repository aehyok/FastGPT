...
const CompanyList = ({ companyId, changeCompany }: { companyId: number }) => {
  ...
  const onclickCreateModel = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/model?companyId=${id}`);
    } catch (err: any) {
      toast({
      ...
      [
        {
          ...
          // )
          list: JSON.parse(localStorage.getItem('companyTable'))
        },
        {
          label: '收藏',
    ...
    >
      <Flex w={'90%'} my={5} mx={'auto'}>
        ...
        <Tooltip label={'新建客服资料库'}>
          <IconButton h={'32px'} icon={<Search2Icon />} aria-label={''} variant={'outline'} />
            ...
            {item.list.map((item) => (
              <Flex
                ...
                {...(companyId == item.id
                  ...
                  : {})}
                onClick={() => {
                  if (item.id === companyId) return;
                  console.log(companyId, item.id);
                  changeCompany(item.id);
                  // router.push(`/model?companyId=${item._id}`);
                }}
              >
                <Image
